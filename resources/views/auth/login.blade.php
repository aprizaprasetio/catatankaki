...
@section('body')
    <div class="p-16 rounded-md bg-gradient-to-r from-blue-600 via-cyan-800 to-blue-900 my-9 drop-shadow-md bg text-slate-900 bg-opacity-60">
        <form class="grid gap-12 p-12 bg-white rounded-md" action="/masuk" method="post">
            @csrf
            ...
            <section class="grid gap-3">
                ...
                @elseif (Session::has('sukses'))
                    <label class="text-sm text-lime-600" for="">Akun berhasil terdaftar!</label>
                @endif
            ...
            <section class="grid gap-3">
                <button class="py-2 text-lg text-white bg-blue-600 rounded-lg shadow-sm hover:bg-blue-100 hover:text-blue-800 outline outline-0 outline-blue-900 hover:outline-1" type="submit">Masuk</button>
                <a class="text-center text-slate-900 hover:text-blue-800" href="/daftar">Saya Pengguna Baru</a>
            </section>
        </form>
    </div>
@endsection